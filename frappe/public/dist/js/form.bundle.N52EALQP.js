...
`;
  ...
  frappe.ui.form.FormViewers.set_users = function(data, type) {
    ...
    const set_and_refresh = () => {
      ...
      if (cur_frm && cur_frm.doc && cur_frm.doc.doctype === doctype && cur_frm.doc.name == docname && cur_frm.viewers) {
  ...
  var ReminderManager = class {
    ...
    create_reminder() {
      ...
      frappe.xcall("frappe.automation.doctype.reminder.reminder.create_new_reminder", {
        ...
        frappe.show_alert(__("Reminder set at {0}", [frappe.datetime.str_to_user(reminder.remind_at)]));
      });
    }
  };
  ...
  frappe.ui.form.Toolbar = class Toolbar {
    ...
    make_menu_items() {
      ...
      this.page.add_menu_item(__("Remind Me"), () => {
...
${this.frm.doctype_layout.client_script}`;
      ...
      function setup_add_fetch(df) {
        ...
        if (is_read_only_field && df.fetch_from && (!df.fetch_if_empty || df.fetch_if_empty && !me.frm.doc[df.fieldname]) && df.fetch_from.indexOf(".") != -1) {
      ...
      if (frappe.model.can_write(this.frm.doctype, this.frm.name)) {
        remove_action = function(target_id) {
          frappe.confirm(__("Are you sure you want to delete the attachment?"), function() {
            ...
            let to_be_removed = me.get_attachments().filter((attachment2) => attachment2.file_name === target_attachment.file_name);
        ...
        if (fieldobj) {
          if (!if_missing || !frappe.model.has_value(me.doctype, me.doc.name, f)) {
            if (frappe.model.table_fields.includes(fieldobj.df.fieldtype) && $.isArray(v)) {
              ...
              v.forEach((d, idx) => {
                let child = frappe.model.add_child(me.doc, fieldobj.df.options, fieldobj.df.fieldname, idx + 1);
...
//# sourceMappingURL=form.bundle.N52EALQP.js.map
